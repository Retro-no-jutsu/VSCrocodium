...
if ($testinstall -eq $True)
{
    ...
    Write-host ""
    Write-host "Attention une installation de VSCrocodium existe deja dans votre dossier utilisateur" -ForegroundColor Red
    ...
    if ($continue2 -eq "o")
    {
        Remove-Item "$($env:USERPROFILE)\VSCrocodium\" -Filter * -Recurse -ErrorAction Ignore
    }
    else 
    {
        exit
...
Write-Host "." -NoNewline
Start-Process -WindowStyle hidden -FilePath "$($env:USERPROFILE)\VSCrocodium\bin\codium.cmd" -wait -ArgumentList "--install-extension `"$($env:USERPROFILE)\downloads\vscrocodium_install\z80-macroasm.vsix`""# Installation de l'extension Z80-macroasm
Start-Process -WindowStyle hidden -FilePath "$($env:USERPROFILE)\VSCrocodium\bin\codium.cmd" -wait -ArgumentList "--install-extension `"$($env:USERPROFILE)\downloads\vscrocodium_install\z80-asm-meter.vsix`"" # Installation de l'extension Z80-asm-meter
Start-Process -WindowStyle hidden -FilePath "$($env:USERPROFILE)\VSCrocodium\bin\codium.cmd" -wait -ArgumentList "--install-extension MS-CEINTL.vscode-language-pack-fr" # Installation de l'extension language FR
Write-Host "." -NoNewline
